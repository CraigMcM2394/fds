...
!Also want to initialise GENISTELA if radiation present- flag for GENISTELA to be added later
IF (RADIATION) CALL INIT_GS
...
MAIN_LOOP: DO
   ...
   ! Apply GENISTELA analysis if desired and radiation has been updated (radiation counter part of mesh info)
   DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   IF (MOD(MESHES(NM)%RAD_CALL_COUNTER,TIME_STEP_INCREMENT)==0) THEN  !<remove .OR. ICYC==1, if init values elsewhere
   GSANLY=.TRUE.
   CYCLE
   ! ELSE GSANLY=.FALSE. is superfluous 
   ENDIF
   ENDDO
   
   IF (GSANLY) CALL GENISTELA
...
SUBROUTINE INIT_GS
! Presume that GENISTELA can access values from here, since part of same program?
! INITIALISE THE CELLULAR STRUCTURE TOO?
! Initialise arrays for storing temperatures and counter
REAL(EB), ALLOCATABLE, DIMENSION(4,:) :: TMP_SURF  ! SURFACE1, SURFACE2, INTERMEDIATE SURFACE1, INTERMEDIATE SURFACE2 
REAL(EB), ALLOCATABLE, DIMENSION(3,:) :: TMP_MBR  ! STEEL, STEEL WITH AXIAL GRADIENT, EFFECTIVE INSULATION TEMPERATURE
REAL(EB) :: RK_OLD, RK_OLD_AX  ! VALUES FOR 1ST RK EXPRESSION FROM PREVIOUS TIMESTEP 
INTEGER :: N_GS ! identifies location in temperature array

!Initial conditions for time-dependent terms
...
SUBROUTINE GENISTELA

! Define parameters used only in GeniSTELA: surface and steel temperatures, emissivity, thermal conductivity, thicknesses,
&! weight factors, densities, specific heats

! Check for parameters not defined here, which still have labels... (e.g. TMP_GAS)
! NEED THIS TO WORK ON A CELL-BY-CELL BASIS

! start off by prescribing values, then I will work out how to import from ASCII file
! many of these require ends 1 and 2, so check if values are different
REAL(EB) :: HTC_MBR1, HTC_MBR2
REAL(EB) :: EMSV_MBR1, EMSV_MBR2
REAL(EB) :: K_PRT1, K_PRT2
REAL(EB) :: K_STEEL
REAL(EB) :: THK_PRT1, THK_PRT2
REAL(EB) :: THK_STEEL
REAL(EB) :: WF_PRT1, WF_PRT2
REAL(EB) :: THM_PEN_DEPTH1, THM_PEN_DEPTH2
REAL(EB) :: TIME_TRANS
REAL(EB) :: RHO_STEEL
REAL(EB) :: RHO_PRT1, RHO_PRT2
REAL(EB) :: SHC_STEEL
REAL(EB) :: SHC_PRT1, SHC_PRT2
! TIMESTEP IS OBTAINED FROM DT; TIME_STEP_INCREMENT IS ASSOCIATED WITH THE RADIATION

INTEGER, PARAMETER :: ITER_MAX = 50
REAL(EB), PARAMETER :: TOL=0.001_EB
REAL(EB) :: TMP_ITER, TMP_SURF1, TMP_SURF2, TMP_SURF1_OLD, TMP_SURF2_OLD, DELTA_X, F_X, FPRIME_X, RK_1ST, RK_2ND,&
RK_1ST_AX, RK_2ND_AX, RK_SCALE, TMP_INT_SURF1, TMP_INT_SURF2, TMP_INT_SURF1_OLD, TMP_INT_SURF2_OLD, TMP_STEEL, TMP_STEEL_AX,&
TMP_STEEL_U, TMP_STEEL_L, TMP_INT_STEEL, TMP_INT_STEEL_AX, TMP_INT_STEEL_U, TMP_INT_STEEL_L, TMP_INSU
INTEGER :: NR_ITER 

! call and define temperatures for numerical reference
TMP_SURF1_OLD = TMP_SURF(1,N_GS)
TMP_SURF1 = TMP_SURF1_OLD !want starting value in NR to be previous solution for expediency
TMP_SURF2_OLD = TMP_SURF(2,N_GS)
TMP_SURF2 = TMP_SURF2_OLD
TMP_INT_SURF1_OLD = TMP_SURF(3,N_GS)
TMP_INT_SURF2_OLD = TMP_SURF(4,N_GS)

TMP_STEEL = TMP_MBR(1,N_GS)
TMP_STEEL_AX = TMP_MBR(2,N_GS)

! update column of temperature arrays
N_GS = N_GS + 1


! NEWTON RAPHSON FOR SURFACE1 TEMPERATURE
DO NR_ITER = 1,ITER_MAX
F_X = BOUND_CONDN (TMP_SURF1,HTC_MBR1,TMP_GAS1,TMP_STEEL,RAD_HEAT_FLUX1,EMSV_MBR1,SIGMA,K_PRT1,THK_PRT1,WF_PRT1)
FPRIME_X = BOUND_CONDN_PRIME (TMP_SURF1,HTC_MBR1,SIGMA,EMSV_MBR1,K_PRT1,WF_PRT1,THK_PRT1) !find f(T) and f'(T)
DELTA_X = F_X / FPRIME_X
TMP_ITER = TMP_SURF1 - DELTA_X

IF (ABS(TMP_SURF1-TMP_ITER) < TOL) EXIT  !stop iterating if within 0.1%

TMP_SURF1 = TMP_ITER ! set new value of surface temperature

END DO
TMP_SURF(1,N_GS) = TMP_SURF1  !store surface temperature for display and guess at next timestep

! NEWTON RAPHSON FOR SURFACE2 TEMPERATURE
DO NR_ITER = 1,ITER_MAX
F_X = BOUND_CONDN (TMP_SURF2,HTC_MBR2,TMP_GAS2,TMP_STEEL,RAD_HEAT_FLUX2,EMSV_MBR2,SIGMA,K_PRT2,THK_PRT2,WF_PRT2)
FPRIME_X = BOUND_CONDN_PRIME (TMP_SURF2,HTC_MBR2,SIGMA,EMSV_MBR2,K_PRT2,WF_PRT2,THK_PRT2) !find f(T) and f'(T)
DELTA_X = F_X / FPRIME_X
TMP_ITER = TMP_SURF2 - DELTA_X

IF (ABS(TMP_SURF2-TMP_ITER) < TOL) EXIT  !stop iterating if within 0.1%

TMP_SURF2 = TMP_ITER ! set new value of surface temperature

END DO
TMP_SURF(2,N_GS) = TMP_SURF2  !store surface temperature for display and guess at next timestep


TMP_INT_STEEL = TMP_STEEL + RK_OLD ! intermediate uses values from previous timestep
TMP_INT_STEEL_AX = TMP_STEEL_AX + RK_OLD_AX !RETURN FOR AXIAL GRADIENT

! NEWTON RAPHSON FOR INTERMEDIATE SURFACE1 TEMPERATURE
DO NR_ITER = 1,ITER_MAX
F_X = BOUND_CONDN (TMP_SURF1,HTC_MBR1,TMP_GAS1,TMP_INT_STEEL,RAD_HEAT_FLUX1,EMSV_MBR1,SIGMA,K_PRT1,THK_PRT1,WF_PRT1)
FPRIME_X = BOUND_CONDN_PRIME (TMP_SURF1,HTC_MBR1,SIGMA,EMSV_MBR1,K_PRT1,WF_PRT1,THK_PRT1) !find f(T) and f'(T)
DELTA_X = F_X / FPRIME_X
TMP_ITER = TMP_INT_SURF1 - DELTA_X

IF (ABS(TMP_INT_SURF1-TMP_ITER) < TOL) EXIT  !stop iterating if within 0.1%

TMP_INT_SURF1 = TMP_ITER ! set new value of surface temperature

END DO
TMP_SURF(3,N_GS) = TMP_INT_SURF1 !store surface temperature for use at next timestep

! NEWTON RAPHSON FOR INTERMEDIATE SURFACE2 TEMPERATURE
DO NR_ITER = 1,ITER_MAX
F_X = BOUND_CONDN (TMP_SURF2,HTC_MBR2,TMP_GAS2,TMP_INT_STEEL,RAD_HEAT_FLUX2,EMSV_MBR2,SIGMA,K_PRT2,THK_PRT2,WF_PRT2)
FPRIME_X = BOUND_CONDN_PRIME (TMP_SURF2,HTC_MBR2,SIGMA,EMSV_MBR2,K_PRT2,WF_PRT2,THK_PRT2) !find f(T) and f'(T)
DELTA_X = F_X / FPRIME_X
TMP_ITER = TMP_INT_SURF2 - DELTA_X

IF (ABS(TMP_INT_SURF2-TMP_ITER) < TOL) EXIT  !stop iterating if within 0.1%

TMP_INT_SURF2 = TMP_ITER ! set new value of surface temperature

END DO
TMP_SURF(4,N_GS) = TMP_INT_SURF2  !store surface temperature for use at next timestep


! RUNGE KUTTA (2ND ORDER) FOR STEEL TEMPERATURE

RK_SCALE = 0.5_EB*DT/(THK_STEEL*RHO_STEEL*SHC_STEEL) ! does the 0.5 need EB?
RK_1ST = ENERGY_BALANCE(HTC_MBR1,TMP_GAS1,TMP_INT_SURF1,TMP_INT_SURF1_OLD,RAD_HEAT_FLUX1,EMSV_MBR1,SIGMA,&
THK_PRT1,WF_PRT1,RHO_PRT1,SHC_PRT1,DT) + ENERGY_BALANCE (HTC_MBR2,TMP_GAS2,TMP_INT_SURF2,TMP_INT_SURF2_OLD,RAD_HEAT_FLUX2,&
EMSV_MBR2,SIGMA,THK_PRT2,WF_PRT2,RHO_PRT2,SHC_PRT2,DT)
RK_2ND = ENERGY_BALANCE(HTC_MBR1,TMP_GAS1,TMP_SURF1,TMP_SURF1_OLD,RAD_HEAT_FLUX1,EMSV_MBR1,SIGMA,&
THK_PRT1,WF_PRT1,RHO_PRT1,SHC_PRT1,DT) + ENERGY_BALANCE (HTC_MBR2,TMP_GAS2,TMP_SURF2,TMP_SURF2_OLD,RAD_HEAT_FLUX2,EMSV_MBR2,SIGMA,&
THK_PRT2,WF_PRT2,RHO_PRT2,SHC_PRT2,DT)

TMP_STEEL = TMP_STEEL + RK_SCALE*(RK_1ST + RK_2ND)

! RUNGE KUTTA FOR AXIAL GRADIENT CORRECTION **STILL NEEDS A SWITCH, MAY NEED TO RECALCULATE VALUES**
! RK_SCALE = 0.5_EB*DT/(THK_STEEL*RHO_STEEL*SHC_STEEL) recalculated if in a switch
RK_1ST_AX = ENERGY_BALANCE(HTC_MBR1,TMP_GAS1,TMP_INT_SURF1,TMP_INT_SURF1_OLD,RAD_HEAT_FLUX1,EMSV_MBR1,SIGMA,&
THK_PRT1,WF_PRT1,RHO_PRT1,SHC_PRT1,DT) + ENERGY_BALANCE (HTC_MBR2,TMP_GAS2,TMP_INT_SURF2,TMP_INT_SURF2_OLD,RAD_HEAT_FLUX2,&
EMSV_MBR2,SIGMA,THK_PRT2,WF_PRT2,RHO_PRT2,SHC_PRT2,DT) &
+ K_STEEL/THK_STEEL*(TMP_INT_STEEL_U-TMP_INT_STEEL) - K_STEEL/THK_STEEL*(TMP_INT_STEEL-TMP_INT_STEEL_L)
RK_2ND_AX = ENERGY_BALANCE(HTC_MBR1,TMP_GAS1,TMP_SURF1,TMP_SURF1_OLD,RAD_HEAT_FLUX1,EMSV_MBR1,SIGMA,&
THK_PRT1,WF_PRT1,RHO_PRT1,SHC_PRT1,DT) + ENERGY_BALANCE (HTC_MBR2,TMP_GAS2,TMP_SURF2,TMP_SURF2_OLD,RAD_HEAT_FLUX2,EMSV_MBR2,SIGMA,&
THK_PRT2,WF_PRT2,RHO_PRT2,SHC_PRT2,DT) + K_STEEL/THK_STEEL*(TMP_STEEL_U-TMP_STEEL) - K_STEEL/THK_STEEL*(TMP_STEEL-TMP_STEEL_L)

TMP_STEEL_AX = TMP_STEEL_AX + RK_SCALE*(RK_1ST + RK_2ND)

CONTAINS
REAL FUNCTION BOUND_CONDN(TMP_FACE,HTC_MBR,TMP_GAS,TMP_STEEL,RAD_HEAT_FLUX,EMSV_MBR,SIGMA,K_PRT,THK_PRT,WF_PRT)

REAL(EB) :: TMP_FACE,HTC_MBR,TMP_GAS,TMP_STEEL,RAD_HEAT_FLUX,EMSV_MBR,SIGMA,K_PRT,THK_PRT,WF_PRT 

BOUND_CONDN = -EMSV_MBR*SIGMA*TMP_FACE**4 - HTC_MBR*TMP_FACE - (K_PRT/(WF_PRT*THK_PRT))*TMP_FACE + HTC_MBR*TMP_GAS + &
RAD_HEAT_FLUX + (K_PRT/(WF_PRT*THK_PRT))*TMP_STEEL

END BOUND_CONDN

REAL FUNCTION BOUND_CONDN_PRIME(TMP_FACE,HTC_MBR,SIGMA,EMSV_MBR,K_PRT,WF_PRT,THK_PRT)

REAL(EB) :: TMP_FACE,HTC_MBR,SIGMA,EMSV_MBR,K_PRT,WF_PRT,THK_PRT

BOUND_CONDN_PRIME = -4._EB*EMSV_MBR*SIGMA*TMP_FACE**3 - HTC_MBR - K_PRT/(WF_PRT*THK_PRT)

END BOUND_CONDN_PRIME

REAL FUNCTION ENERGY_BALANCE(HTC_MBR,TMP_GAS,TMP_FACE,TMP_FACE_OLD,RAD_HEAT_FLUX,EMSV_MBR,SIGMA,THK_PRT,WF_PRT,RHO_PRT,SHC_PRT,DT)
REAL(EB) :: HTC_MBR,TMP_GAS,TMP_FACE,TMP_FACE_OLD,RAD_HEAT_FLUX,EMSV_MBR,SIGMA,WF_PRT,RHO_PRT,SHC_PRT,DT

ENERGY_BALANCE = HTC_MBR*(TMP_GAS - TMP_FACE) + RAD_HEAT_FLUX - EMSV_MBR*SIGMA*TMP_FACE**4 & 
-(TMP_FACE - TMP_FACE_OLD)*(THK_PRT*WF_PRT*RHO_PRT*SHC_PRT/DT)
